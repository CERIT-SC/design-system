...
class Path {
  ...
  update() {
    ...
    if (!this.finished && Math.random() < MOVE_PROBABILITY) {
      ...
      if (
        this.y < 0 ||
        this.y >= this.height ||
        this.x > this.width + GRID_SIZE * 10
      ) {
        this.finished = true;
        return;
      }
  ...
  draw() {
    ...
    for (let i = 0; i < this.points.length - 1; i++) {
      ...
      this.ctx.strokeStyle = `rgba(${PATH_COLOR}, ${String(opacity)})`;
      ...
      if (isDot && opacity > 0.05) {
        ...
        this.ctx.fillStyle = `rgba(${PATH_COLOR}, ${String(opacity)})`;
        this.ctx.shadowBlur = 0;
        this.ctx.fill();
      }
    }
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      if (Math.random() < SPAWN_RATE && paths.length < MAX_PATHS) {
        paths.push(new Path(width, height, ctx));
      }
