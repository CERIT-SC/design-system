  ...
  <div className="flex flex-col gap-8 w-full">
    ...
    <div className="rounded-lg border border-dashed overflow-hidden">
      <Header className="bg-linear-to-r from-primary/90 to-secondary/90 border-transparent">
        <HeaderContent>
          <HeaderLeft>
            <img src="/e-INFRA_logo_RGB_bila.svg" alt="e-INFRA CZ" className="h-12 w-auto" />
            <NavigationMenu>
              <NavigationMenuList>
                <NavigationMenuItem>
...
- The previews above swap the e-INFRA CZ logo by theme with a `dark:hidden` / `hidden dark:block` image pair. Hidden elements are excluded from the accessibility tree, so both images carry the same `alt` and the logo is announced exactly once in either theme. The gradient header is dark in both themes, so it always takes the white logo.
