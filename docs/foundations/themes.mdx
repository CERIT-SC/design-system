...
import '@e-infra/design-system/elter_setup.css';  // ELTER theme
```

## Theme Fonts

The branding themes set a font family, but the library **does not ship the font
files**. Each theme reads a CSS variable that your app is responsible for
defining:
...
If the variable is missing the theme degrades gracefully — it falls back to a
locally installed copy of the font, then to the system sans-serif — but you will
not get the intended branding. To load the fonts properly:
...
> **Gotcha if you write your own theme:** always give `var()` a fallback inside a
> `font-family` stack — `var(--font-x, "X"), sans-serif`, never `var(--font-x),
> sans-serif`. An undefined custom property with no fallback makes the entire
> declaration invalid at computed-value time, so the browser throws the whole
> stack away rather than continuing to the next entry. On `<html>` that means
> falling all the way back to the default serif (Times New Roman).

## Theme Persistence

Theme preferences are automatically persisted in `localStorage`:

- `theme-branding`: `"default"`, `"eosc"` or `"elter"`
...
[eosc_setup.css](https://github.com/CERIT-SC/design-system/blob/main/lib_public/eosc_setup.css),
and [elter_setup.css](https://github.com/CERIT-SC/design-system/blob/main/lib_public/elter_setup.css)
